...
public class OutputRaw {
	...
	{
		if (Main.getInstance().getLogger().isDebug()) Main.getInstance().getLogger().debug(line);
		else
		{
			ArrayList<String> strings = new ArrayList<>(Arrays.asList(line.split(" ")));
			if (strings.get(0).equalsIgnoreCase("PRIVMSG"))
				Main.getInstance().getLogger().log(String.format("[O] Reply %s %s", strings.get(1), ArrayUtils.getTheRestArgsAsString(strings, 2)));
		}
	}
